...
export function getTokenMetadataIndex(asset: AssetInfo, ibcMap: IBCMap, native_chain: string): string | undefined {
    ...
    if(isIBCHash(tokenValue) || isFactoryHash(tokenValue)) {
        ...
        if(ibcMappedToken) {
            ...
            index = getIndex(ibcMappedToken.origin.denom, ibcMappedToken.origin.chain);
        }else {
